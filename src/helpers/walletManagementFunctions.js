...
export const handleConnectWallet = async () => {
  ...
  try {
    ...
    if (signer) {
      ...
      console.log('handleConnectWallet humanizeAddress _address - ', humanizeAddress(_address));
      subscribeProvider(signer);

      const balance = await signer.getBalance();
      const balanceInBNB = utils.formatEther(balance);
      console.log('balanceInBNB...', balanceInBNB);

      return [_address, balanceInBNB];
    }
  } catch (err) {
...
const getProviderOrSigner = async (needSigner = false) => {
  ...
  if (!METAMASK_CONFIG.SUPPORTED_CHAID_IDS.includes(chainId)) {
    console.log('Change network to Binance Smart Chain Mainnet testnet');
    return null;
  }
